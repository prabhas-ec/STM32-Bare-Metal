...
uint8_t SPI_ReceiveDataIT(SPI_Handle_t *pSPIHandle, uint8_t *pRxBuffer, uint32_t Len)
{
	...
	if(state != SPI_BUSY_IN_RX)
	{
		...
		// 2. Mark the SPI state as busy in transmission so that no other code can take over same SPI peripheral until reception is over
	...
	// 4. Data reception will be handled by ISR code
	return state;
}
